import 'package:flutter/material.dart';

class AppColors {

  AppColors._(); // costruttore privato così non devo instanziarla nell'app

  static const background = Color(0xFFFFFFFF);
  static const buttonEnabled = Color(0xFF373737);
  static const fieldText = Color(0xCC373737);
  static const iconFocused = Color(0xFF373737);
  static const iconUnfocused = Color(0xFFFFFFFF);
  static const searchBar = Color(0x7FDDC4CA);
  static const tapBarBackground = Color(0xFFF87594);
  static const image = Color(0xFFCE91A0);
  static const textPrimary = Color(0xFF373737);
  static const buttonText = Color(0xFFFFFFFF);
  static const addCard = Color(0xFFD1C4C3);
  static const registrationBg = Color(0xFFC9AEB5);

  static const aperol = Color(0xFFFFCACC);
  static const campari = Color(0xFFE8B9BB);
  static const prosecco = Color(0xFFE7CCA0);
  static const soda = Color(0xFFE8E8E8);
  static const tequila = Color(0xFFDBFDFF);
  static const vodka = Color(0xFFC6DBFF);
  static const rumS = Color(0xFFB07D5F);
  static const gin = Color(0xFFC6C5C1);
  static const rumB = Color(0xFFAAB0A2);
  static const triple = Color(0xFFB78A76);
  static const vermouth = Color(0xFF938076);

  static const bloody = Color(0xFFDB8573);
  static const margarita = Color(0xFFC9CCA4);
  static const mojito = Color(0xFFA7A388);
  static const negroni = Color(0xFFE8BC97);

  static const arancia = Color(0xFFE7BB73);
  static const basilico = Color(0xFF93B88B);
  static const gingerbeer = Color(0xFFF8E3C8);
  static const lime = Color(0xFFDDF895);
  static const limone = Color(0xFFFEFBBC);
  static const menta = Color(0xFFB9CFA4);
  static const pepe = Color(0xFFD0B4A7);

  static const sale = Color(0xFFDCDCDE);
  static const sciroppozucchero = Color(0xFFEBE5E3);
  static const succolime = Color(0xFF98A5A7);
  static const succolimone = Color(0xFFEBE8C6);
  static const succopomodoro = Color(0xFFCB948B);
  static const tabasco = Color(0xFFC3705E);
  static const worch = Color(0xFFFEE9B9);
  static const zucchero = Color(0xFFD9D7D8);
  static const zuccherocanna = Color(0xFFF3DCCC);

}